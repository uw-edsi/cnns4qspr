...
"""
import sys
sys.path.append("/gscratch/pfaendtner/Chowdhury/cnns4qspr")
...
import numpy as np
import importlib
from cnns4qspr.get_param_groups import get_param_groups
...
class CNNTrainer():
    ...
    def train(self,
        ...
        with open ('params.txt', 'w') as g:
            g.write(str(self.param_groups))
        with open ('network.txt', 'w') as h:
            h.write(str(self.network))
        if self.loaded:
            self.network.load_state_dict(self.current_state['state_dict'])
        ...
        for epoch in range(epochs):
            ...
            #self.history['train_loss'].append(np.mean(train_total_losses))
            #self.history['train_predictor_loss'].append(np.mean(val_predictor_losses))
            ...
            if store_best:
                if store_criterion == 'val_acc':
                    if np.mean(train_accs) > self.best_acc:
                ...
                elif store_criterion == 'train_loss':
                    if np.mean(train_total_losses) < self.best_loss:
                        self.best_loss = np.mean(train_total_losses)
                        self.best_state['state_dict'] = copy.deepcopy(self.network.state_dict())
                        self.best_state['optimizer'] = copy.deepcopy(self.optimizer.state_dict())
                        self.best_state['epoch'] = self.n_epochs
