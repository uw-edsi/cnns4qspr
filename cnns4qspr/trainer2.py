...
class CNNTrainer():
    ...
    def train(self,
        ...
        if verbose:
            sys.stdout.write('\r'+'loading data...\n')
            sys.stdout.flush()
        #val_data = CustomIterableDataset(data[val_indices, :])
        if verbose:
            sys.stdout.write('\r'+'loading data...\n')
            sys.stdout.flush()
        #data = None
        if verbose:
            sys.stdout.write('\r'+'Data deleted...\n')
            sys.stdout.flush()
        train_sampler = SubsetRandomSampler(train_indices)
        val_sampler = SubsetRandomSampler(val_indices)
        train_loader = torch.utils.data.DataLoader(data, batch_size=self.args.batch_size,
                                                   num_workers=1, sampler=train_sampler,
                                                   pin_memory=True)
        #train_data = None
        if verbose:
            sys.stdout.write('\r'+'Train Data deleted...\n')
            sys.stdout.flush()
        val_loader = torch.utils.data.DataLoader(data, batch_size=self.args.batch_size,
